...
global username, types, style, brand, pwd
cart = []
...
def getCatalogue():
    ...
    with con:
        ...
        for row in rows:
            ...
            output+='<td><img src=/static/products/'+row[7]+' width=200 height=200></img></td><td><a href=\'AddCart?pid='+str(row[0])+'\'><font size="" color="black">Click Here</a></td>'
            output+='<td><a href=\'ViewCart?pid='+str(row[0])+'\'><font size="" color="black">Click Here</a></td></tr>'
...
def ViewCart(request):
    if request.method == 'GET':
        global cart
        output = '<table border=1 align=center width=100%><tr><th>Product ID</th><th>Product Name</th><th>Cost</th><th>Removed Item</th></tr>'
        for i in range(len(cart)):
            pid = cart[i]
            con = pymysql.connect(host='127.0.0.1',port = 3306,user = 'root', password = 'root', database = 'ApparelApp',charset='utf8')
            with con:
                ...
                for row in rows:
                    ...
                    output+='<td><a href=\'RemoveCart?pid='+str(pid)+'\'><font size="" color="black">Click Here</a></td></tr>'
